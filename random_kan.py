...
import kan_species as race
import character_sheet as npc
...
class Randomize_Gender:

    def __init__(self):

        self.The_List = re.Kan_gender
        self.Result = ""

    def Randomizor(self):
        Limiter = len(self.The_List)
        Oddity_Checker = rd.randint(1,100)
        if Oddity_Checker < 100:
            Halt = 1
        else:
            Halt = Limiter
        final_number = rd.randint(0,Halt)
        self.Result = re.Kan_gender[final_number]

Ran_Name = ""

Cal_Gender = Randomize_Gender()
Cal_Gender.Randomizor()
Ran_Gender = Cal_Gender.Result
if cf.Debug.DebugMode == 1:
    print(Cal_Gender.Result)
